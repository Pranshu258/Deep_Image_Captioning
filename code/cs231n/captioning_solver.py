...
class CaptioningSolver(object):
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self.model = model
        self.data = data
        self.full_data = load_coco_data(pca_features=True)
    ...
    def evaluate_model(self):
        ...
        for split in ['train', 'val']:
            minibatch = sample_coco_minibatch(self.full_data, split=split, batch_size=1000)
            gt_captions, features, urls = minibatch
            gt_captions = decode_captions(gt_captions, self.full_data['idx_to_word'])

            sample_captions = self.model.sample(features)
            sample_captions = decode_captions(sample_captions, self.full_data['idx_to_word'])
        ...
        print(BLEUscores['train'], BLEUscores['val'])
        return BLEUscores['train'], BLEUscores['val']
    ...
    def train(self):
        ...
        for t in range(num_iterations):
            ...
            if t % self.print_every == 0:
                tr, vl = self.evaluate_model()
                if tr >= 0.3 and vl >= 0.3:
                    return
